...
const ProjectsView = (function() {
    ...
    /**
     * Get tasks count for a project (by UUID)
     */
    function getProjectTasksCount(projectId) {
        const tasks = typeof AppState !== 'undefined' ? AppState.tasks || [] : [];
        return tasks.filter(t => t.project === projectId || t.project_id === projectId).length;
    }
    ...
    function renderProjectCard(project) {
        const tasksCount = getProjectTasksCount(project.id);
    ...
    function openProject(projectId, projectName) {
        const tasksCount = getProjectTasksCount(projectId);

        if (tasksCount === 0) {
            // Project is empty - show detail view
            showProjectDetail(projectId, projectName);
        } else {
            // Project has tasks - navigate to tasks view with filter by ID
            ...
            setTimeout(() => {
                if (typeof Projects !== 'undefined' && Projects._selectProject) {
                    Projects._selectProject(projectId);
                }
            }, 100);
        }
    }
