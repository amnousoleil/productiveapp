...
export class AIReportsService {
  ...
  async getReports(
    workspaceId: UUID,
    userId: UUID,
    params: AIReportListParams
  ): Promise<{ reports: AIReport[]; total: number }> {
    const page = params.page || 1;
    const limit = params.limit || 20;
    const offset = (page - 1) * limit;

    let conditions = sql`workspace_id = ${workspaceId} AND user_id = ${userId}`;

    if (params.report_type) {
      conditions = sql`${conditions} AND report_type = ${params.report_type}`;
    }

    if (params.from) {
      conditions = sql`${conditions} AND created_at >= ${params.from}`;
    }

    if (params.to) {
      conditions = sql`${conditions} AND created_at <= ${params.to}`;
    }
    ...
    return {
      reports: reports as unknown as AIReport[],
   ...
   */
  async getReportById(reportId: UUID, workspaceId: UUID): Promise<AIReport | null> {
    const result = await sql`
      SELECT * FROM ai_reports
      WHERE id = ${reportId} AND workspace_id = ${workspaceId}
    `;
    return (result[0] as AIReport) || null;
  }

  /**
   * Generate meta-synthesis (analysis of multiple reports)
   */
  async generateMetaSynthesis(
    workspaceId: UUID,
    userId: UUID,
    input: MetaSynthesisInput
  ): Promise<AIReport> {
    ...
    if (input.report_ids && input.report_ids.length > 0) {
      ...
      `;
      reports = result as unknown as AIReport[];
    } else {
  ...
  async getVisualizationData(
  ...
  ): Promise<AIReportVisualization> {
    const { start } = this.getPeriodDates(periodType);

    // Tasks by status
    const tasksByStatus = await sql`
    ...
    // Weekly comparison
    const weeklyComparison = await sql`
      SELECT
    ...
    return {
      tasksByStatus: this.formatChartData(
        tasksByStatus,
        'status',
        'count',
        ['#10b981', '#f59e0b', '#3b82f6', '#8b5cf6']
      ),
      productivityTrend: this.formatLineChartData(
        dailyProductivity,
        'day',
        'count',
        'Tâches complétées'
      ),
      projectsProgress: this.formatChartData(
        projectsProgress,
        'name',
        'progress',
        ['#3b82f6', '#8b5cf6', '#ec4899', '#f97316', '#10b981']
      ),
      priorityDistribution: this.formatChartData(
        priorityDist,
        'priority',
        'count',
        ['#ef4444', '#f59e0b', '#10b981']
      ),
      weeklyComparison: this.formatLineChartData(
        weeklyComparison,
        'week',
        'count',
        'Semaine'
  ...
  private buildAIContext(
    ...
    const tasksList = tasks as Array<{id: string; title: string; status: string; priority: number; project_id?: string; created_at: string; completed_at?: string}>;
    const projectsList = projects as Array<{id: string; name: string; status?: string; progress: number}>;
...
`;
  ...
  private buildPrompt(
    ...
    const basePrompt = `Analyse les données suivantes et génère un rapport ${
      reportType === 'audit' ? "d'audit comportemental" : 'de productivité'
    }:\n\n${context}`;
  ...
  private getSystemPrompt(reportType: AIReportType): string {
    if (reportType === 'audit') {
...
Sois concis et pratique. Réponds en français.`;
  ...
  private getMetaSynthesisSystemPrompt(): string {
...
Réponds en français.`;
  ...
  private buildSynthesisContext(reports: AIReport[]): string {
  ...
  private buildMetaSynthesisPrompt(context: string, focusAreas?: string[]): string {
...
et propose un plan d'action stratégique pour maximiser le potentiel de cette personne.`;
  ...
  private parseAIResponse(content: string, _reportType: AIReportType): {
    analysis: string;
  ...
  private calculateAIScore(metrics: ReportMetrics, parsed: { recommendations: string[]; strengths: string[]; weaknesses: string[] }): number {
    let score = 0;
  ...
  private calculateEvolutionScore(reports: AIReport[]): number {
    if (reports.length < 2) return 50;
  ...
  private generateTitle(reportType: AIReportType, periodType: PeriodType): string {
    const periodLabel = {
      week: 'Semaine',
      month: 'Mois',
      year: 'Année',
  ...
  private formatChartData(
    ...
    const items = data as Array<Record<string, string | number>>;
    return {
