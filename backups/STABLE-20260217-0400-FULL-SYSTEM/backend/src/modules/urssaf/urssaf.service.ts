...
let pool: Pool;
export const initURSSAFService = (p: Pool): void => { pool = p; };

// Taux URSSAF micro-entrepreneur 2026 (approximatifs)
const RATES: Record<string, { base: number; formation: number; cfe: number }> = {
...
export const simulateCotisations = async (_wid: string, params: { ca: number; activity_type: string; acre?: boolean; quarter?: number; year?: number }) => {
...
export const getDeclarations = async (wid: string) => {
  return (await pool.query('SELECT * FROM fiscal_declarations WHERE workspace_id=$1 ORDER BY year DESC, quarter DESC', [wid])).rows;
};

export const createDeclaration = async (wid: string, mid: string, data: { quarter: number; year: number; activity_type: string; chiffre_affaires: number; acre?: boolean }) => {
  const sim = await simulateCotisations(wid, { ca: data.chiffre_affaires, activity_type: data.activity_type, acre: data.acre, quarter: data.quarter, year: data.year });
  return (await pool.query(
    `INSERT INTO fiscal_declarations (workspace_id, member_id, quarter, year, activity_type, chiffre_affaires, cotisations_amount, net_amount, status)
     ...
     RETURNING *`,
    [wid, mid, data.quarter, data.year, data.activity_type, data.chiffre_affaires, sim.cotisations.total, sim.net_after_cotisations]
  )).rows[0];
};

export const updateDeclaration = async (wid: string, id: string, data: { status?: string; notes?: string }) => {
  const fields: string[] = []; const vals: any[] = []; let pi = 1;
  if (data.status) { fields.push(`status=$${pi++}`); vals.push(data.status); }
  if (data.notes !== undefined) { fields.push(`notes=$${pi++}`); vals.push(data.notes); }
  if (data.status === 'submitted') { fields.push(`submitted_at=$${pi++}`); vals.push(new Date().toISOString()); }
  if (!fields.length) return (await pool.query('SELECT * FROM fiscal_declarations WHERE id=$1 AND workspace_id=$2', [id, wid])).rows[0];
  vals.push(id, wid);
...
export const getAnnualSummary = async (wid: string, year: number) => {
  ...
  const totalCA = decls.reduce((s: number, d: any) => s + parseFloat(d.chiffre_affaires || 0), 0);
  const totalCot = decls.reduce((s: number, d: any) => s + parseFloat(d.cotisations_amount || 0), 0);
  const totalNet = decls.reduce((s: number, d: any) => s + parseFloat(d.net_amount || 0), 0);
...
export const autoCalculateFromInvoices = async (wid: string, quarter: number, year: number) => {
  const startMonth = (quarter - 1) * 3 + 1;
  const endMonth = startMonth + 2;
  const startDate = `${year}-${String(startMonth).padStart(2, '0')}-01`;
  const endDate = `${year}-${String(endMonth).padStart(2, '0')}-${endMonth === 2 ? 28 : [4, 6, 9, 11].includes(endMonth) ? 30 : 31}`;
  const r = await pool.query(
    `SELECT COALESCE(SUM(montant_ttc), 0) AS total FROM invoices WHERE workspace_id=$1 AND type='income' AND status='paid' AND date_facture >= $2 AND date_facture <= $3`,
