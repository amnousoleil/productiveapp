...
import {
  BankTransaction,
  AutoMatchResult, ReconciliationSummary,
  Department, CreateDepartmentDTO, UpdateDepartmentDTO,
  BudgetLine, CreateBudgetDTO, BudgetOverview, DepartmentBudgetSummary,
  AccountingAlert, AlertCounts,
  CompanySettings,
  BalanceSheet, ProfitLoss, ProfitLossLine,
  CashFlowPrediction, CashFlowForecast, AnomalyDetectionResult, SmartCategorization,
  FECEntry, FECGenerationResult,
  PaginatedResponse, Invoice, CreateInvoiceDTO
} from './accounting.types.js';

type Req = Request<{ workspaceId: string; id?: string; txId?: string; invoiceId?: string; departmentId?: string; itemId?: string }>;
 ...
 */
export const importBankTransactions = async (req: Req, res: Response): Promise<void> => {
  try {
    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (cols.length < 3) { errors.push(`Ligne ${i + 1}: nombre de colonnes insuffisant`); continue; }
        ...
        try {
          ...
          if (isNaN(amount)) { errors.push(`Ligne ${i + 1}: montant invalide`); continue; }
 ...
 */
export const listBankTransactions = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
    ...
    if (req.query.min_amount) {
      ...
      params.push(parseFloat(req.query.min_amount as string));
    }
    if (req.query.max_amount) {
      whereClause += ` AND bt.amount <= $${paramIndex++}`;
      params.push(parseFloat(req.query.max_amount as string));
    }
    if (req.query.search) {
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*) FROM bank_transactions bt ${whereClause}`,
      params
    );
    const total = parseInt(countResult.rows[0].count, 10);

    params.push(limit, offset);
    const result = await pool.query<BankTransaction>(
      `SELECT bt.*, i.fournisseur as matched_invoice_name, bt.category_suggestion as category_name
    ...
    } as PaginatedResponse<BankTransaction>);
  } catch (error) {
 ...
 */
export const matchBankTransaction = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId, txId, invoiceId } = req.params;

    // Verifier que la transaction existe
    const txResult = await pool.query(
      'SELECT id FROM bank_transactions WHERE id = $1 AND workspace_id = $2',
      [txId, workspaceId]
    );
    if (!txResult.rows[0]) {
    ...
    const invResult = await pool.query(
      'SELECT id FROM invoices WHERE id = $1 AND workspace_id = $2',
 ...
 */
export const autoMatchBankTransactions = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;

    // Recuperer les transactions non rapprochees
    const unmatchedTx = await pool.query<BankTransaction>(
    ...
    const unmatchedInv = await pool.query<Invoice>(
      ...
      await pool.query(
        `UPDATE bank_transactions
         SET matched_invoice_id = $1, is_reconciled = true,
             match_confidence = $2
         WHERE id = $3`,
        [match.invoice_id, match.confidence, match.transaction_id]
      );
    }
 ...
 */
export const getUnreconciledTransactions = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
    ...
    const unreconciledResult = await pool.query<BankTransaction>(
      `SELECT * FROM bank_transactions
       WHERE workspace_id = $1 AND is_reconciled = false
       ORDER BY transaction_date DESC
       LIMIT 100`,
 ...
 */
export const listDepartments = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
 ...
 */
export const createDepartment = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
    ...
    const result = await pool.query<Department>(
      ...
      RETURNING *`,
      [workspaceId, data.name, data.code, data.manager_name || null]
    );
 ...
 */
export const updateDepartment = async (req: Req, res: Response): Promise<void> => {
  try {
    ...
    const allowedFields: (keyof UpdateDepartmentDTO)[] = ['name', 'code', 'manager_name', 'is_active'];
 ...
 */
export const deleteDepartment = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId, id } = req.params;
 ...
 */
export const getBudgets = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
    ...
    const byDept: DepartmentBudgetSummary[] = deptResult.rows.map(r => {
 ...
 */
export const setBudget = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
 ...
 */
export const getBudgetVariance = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
    const year = req.query.year ? parseInt(req.query.year as string, 10) : new Date().getFullYear();
    ...
    const result = await pool.query(
      `SELECT
       ...
       ${whereClause}
       ORDER BY d.name, b.month NULLS FIRST`,
      params
    );
 ...
 */
export const getDepartmentBudget = async (req: Req, res: Response): Promise<void> => {
  try {
    ...
    const budgetResult = await pool.query<BudgetLine>(
      `SELECT b.*, c.name as category_name
       ...
       ORDER BY b.month NULLS FIRST, c.name`,
      [workspaceId, departmentId, year]
    );

    const totalAllocated = budgetResult.rows.reduce((s, r) => s + parseFloat(r.budget_amount as unknown as string), 0);
    const totalActual = budgetResult.rows.reduce((s, r) => s + parseFloat(r.actual_amount as unknown as string), 0);
    ...
    res.json({
      ...
      lines: budgetResult.rows.map(r => ({
        ...r,
        budget_amount: parseFloat(r.budget_amount as unknown as string),
        actual_amount: parseFloat(r.actual_amount as unknown as string)
      }))
    });
  } catch (error) {
 ...
 */
export const listAlerts = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
 ...
 */
export const markAlertRead = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId, id } = req.params;
 ...
 */
export const dismissAlert = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId, id } = req.params;
 ...
 */
export const generateAlerts = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
    const generated: AccountingAlert[] = [];

    // 1. Factures en retard
    const overdueResult = await pool.query<Invoice>(
      `SELECT id, fournisseur, montant_ttc, date_echeance
       FROM invoices
    ...
    for (const inv of overdueResult.rows) {
      ...
      const existingAlert = await pool.query(
        `SELECT id FROM accounting_alerts
         WHERE workspace_id = $1 AND type = 'overdue_invoice' AND related_entity_id = $2 AND is_dismissed = false`,
        [workspaceId, inv.id]
      );

      if (!existingAlert.rows[0]) {
        const alertResult = await pool.query<AccountingAlert>(
          ...
          [
            ...
            `La facture de ${inv.fournisseur} (${parseFloat(inv.montant_ttc as unknown as string).toFixed(2)} EUR) est en retard de ${daysDue} jours.`,
            inv.id,
    ...
    const budgetResult = await pool.query(
      `SELECT b.department_id, d.name as department_name,
       ...
       HAVING SUM(b.actual_amount) > SUM(b.budget_amount) * 0.9`,
      [workspaceId, new Date().getFullYear()]
    );
    ...
    for (const dept of budgetResult.rows) {
      ...
      const existingAlert = await pool.query(
        ...
        [workspaceId, utilization > 100 ? 'budget_exceeded' : 'budget_warning', dept.department_id]
      );
 ...
 */
export const getCompanySettings = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;

    const result = await pool.query<CompanySettings>(
      'SELECT * FROM company_settings WHERE workspace_id = $1',
      [workspaceId]
    );

    if (!result.rows[0]) {
      // Creer les parametres par defaut
      const defaultResult = await pool.query<CompanySettings>(
 ...
 */
export const updateCompanySettings = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
    const data = req.body;
 ...
 */
export const uploadLogo = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
    const file = req.file;
 ...
 */
export const getBalanceSheet = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
    const year = req.query.year ? parseInt(req.query.year as string, 10) : new Date().getFullYear();
 ...
 * Compte de resultat
 */
export const getProfitLoss = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
    const year = req.query.year ? parseInt(req.query.year as string, 10) : new Date().getFullYear();
    ...
    const revenue: ProfitLossLine[] = revenueResult.rows.map(r => ({
    ...
    const expenses: ProfitLossLine[] = expenseResult.rows.map(r => ({
      category_id: r.category_id,
      label: r.label,
      amount: parseFloat(r.amount),
      count: parseInt(r.count, 10)
 ...
 */
export const getCashFlowStatement = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
    const year = req.query.year ? parseInt(req.query.year as string, 10) : new Date().getFullYear();
 ...
 */
export const predictCashFlow = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
 ...
 */
export const aiCategorize = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
    ...
    await pool.query(
      'SELECT id, slug, name, type FROM accounting_categories WHERE workspace_id = $1',
      [workspaceId]
    );

    // Analyser l'historique pour trouver des patterns similaires
    const similarResult = await pool.query(
      `SELECT c.slug, c.name, COUNT(*) as frequency
       FROM invoices i
       ...
       LIMIT 5`,
      [workspaceId, `%${description.split(' ')[0]}%`]
    );
    ...
    const result: SmartCategorization = {
 ...
 */
export const detectAnomalies = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
    ...
    const avgResult = await pool.query(
      `SELECT AVG(montant_ttc) as avg_amount, STDDEV(montant_ttc) as stddev_amount
    ...
    if (avgAmount > 0 && stddev > 0) {
      ...
      const outliersResult = await pool.query<Invoice>(
        `SELECT id, fournisseur, montant_ttc, type
         FROM invoices
         WHERE workspace_id = $1 AND montant_ttc > $2
         ORDER BY montant_ttc DESC LIMIT 10`,
        [workspaceId, outlierThreshold]
      );

      for (const inv of outliersResult.rows) {
        const deviation = ((parseFloat(inv.montant_ttc as unknown as string) - avgAmount) / avgAmount) * 100;
        anomalies.push({
          type: 'unusual_amount',
          severity: deviation > 500 ? 'critical' : 'warning',
          description: `Facture ${inv.fournisseur} avec montant anormalement eleve: ${parseFloat(inv.montant_ttc as unknown as string).toFixed(2)} EUR (moyenne: ${avgAmount.toFixed(2)} EUR)`,
          entity_id: inv.id,
          entity_type: 'invoice',
          expected_value: avgAmount,
          actual_value: parseFloat(inv.montant_ttc as unknown as string),
    ...
    const duplicatesResult = await pool.query(
      `SELECT a.id as id_a, b.id as id_b, a.fournisseur, a.montant_ttc,
    ...
    const uncategorizedResult = await pool.query(
      `SELECT COUNT(*) as count
       FROM invoices
       WHERE workspace_id = $1 AND category_id IS NULL`,
      [workspaceId]
    );
 ...
 */
export const exportFEC = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
    const { year } = req.body;
    ...
    const invoicesResult = await pool.query<Invoice>(
      `SELECT i.*, c.name as category_name, c.slug as category_slug
       ...
       ORDER BY i.date_facture, i.created_at`,
      [workspaceId, year]
    );

    // Recuperer les parametres de la societe
    const settingsResult = await pool.query<CompanySettings>(
      'SELECT * FROM company_settings WHERE workspace_id = $1',
      [workspaceId]
    );
    ...
    const fecContent = [
      fecHeaders.join('\t'),
      ...entries.map(e => fecHeaders.map(h => e[h as keyof FECEntry]).join('\t'))
    ].join('\n');
    ...
    const result: FECGenerationResult = {
      filename,
      filepath,
 ...
 */
export const exportTVADeclaration = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
 ...
 */
export const markInvoicePaid = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId, id } = req.params;
    ...
    if (result.rows[0].contact_id) {
      await pool.query(
        `UPDATE contacts
         ...
         updated_at = NOW()
         WHERE id = $1 AND workspace_id = $2`,
        [result.rows[0].contact_id, workspaceId]
      );
    }
 ...
 */
export const sendInvoice = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId, id } = req.params;

    const invoice = await pool.query<Invoice>(
    ...
    const result = await pool.query<Invoice>(
      `UPDATE invoices
       ...
       WHERE id = $2 AND workspace_id = $3
       RETURNING *`,
      [new Date().toISOString().split('T')[0], id, workspaceId]
    );
 ...
 */
export const sendReminder = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId, id } = req.params;
 ...
 */
export const batchScanInvoices = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
 ...
 */
export const getOverdueInvoices = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;

    const result = await pool.query<Invoice & { days_overdue: number }>(
      `SELECT i.*,
        c.name as category_name,
    ...
    const totalOverdue = result.rows.reduce((s, r) => s + parseFloat(r.montant_ttc as unknown as string), 0);
 ...
 */
export const createDocument = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId } = req.params;
    const data = req.body;
 ...
 */
export const convertQuoteToInvoice = async (req: Req, res: Response): Promise<void> => {
  try {
    const { workspaceId, id } = req.params;

    const quoteResult = await pool.query<Invoice>(
      `SELECT * FROM invoices
       WHERE id = $1 AND workspace_id = $2 AND document_type = 'quote'`,
      [id, workspaceId]
    );
    ...
    await pool.query(
      `UPDATE invoices
       SET status = 'validated',
           ...
           updated_at = NOW()
       WHERE id = $3`,
      [newInvoice.id, new Date().toISOString().split('T')[0], id]
    );
