...
export const initContactsService = (dbPool: Pool): void => {
  pool = dbPool;
};
...
export const getContactById = async (
  ...
  try {
    ...
    return {
      ...result.rows[0],
      total_invoiced: parseFloat(result.rows[0].total_invoiced as unknown as string),
      total_paid: parseFloat(result.rows[0].total_paid as unknown as string)
    };
  } catch (error) {
...
export const updateContact = async (
  ...
  try {
    ...
    const allowedFields: (keyof UpdateContactDTO)[] = [
 ...
 */
export const updateContactTotals = async (
  workspaceId: string,
  contactId: string
): Promise<void> => {
  try {
