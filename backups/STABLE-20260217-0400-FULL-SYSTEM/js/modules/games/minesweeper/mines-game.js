...
const MinesGame = (function() {
    ...
    function render() {
        if (!container) return;
        container.innerHTML = `<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:16px;"><div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;justify-content:center;">${Object.entries(CFGS).map(([k,v])=>`<button class="games-btn" onclick="MinesGame.changeDiff('${k}')" style="${k===currentDiff?'background:var(--accent-primary,#7c3aed);color:white;':''}">${v.name}</button>`).join('')}<button class="games-btn" onclick="MinesGame.restart()">🔄 Nouveau</button><div style="display:flex;gap:16px;font-size:13px;color:var(--text-secondary);">💣 <span id="mines-count">${mines}</span> &nbsp; ⏱ <span id="mines-timer">0:00</span></div></div><div id="mines-board" style="overflow:auto;"><div class="mines-grid" id="mines-grid" style="grid-template-columns:repeat(${cols},32px);"></div></div><div style="font-size:12px;color:var(--text-secondary)">Clic gauche = révéler · Clic droit = drapeau</div></div>`;
        renderBoard();
    }
