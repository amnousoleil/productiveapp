/**
 * NOTES GRAPH VIEW - Orchestrator for 3D knowledge graph
 * ProductiveApp v5.0
 *
 * Manages:
 * - Fullscreen modal with glassmorphism
 * - Toolbar (Sync, Reset, Labels, Auto-Rotate, Auto-Link)
 * - API integration with backend graph system
...
const NotesGraphView = (function() {
    ...
    function open() {
        ...
        if (typeof NotesGraph3D !== 'undefined') {
            NotesGraph3D.init(containerEl);
        }
    ...
    function toggleLabels() {
        if (typeof NotesGraph3D !== 'undefined') {
            NotesGraph3D.toggleLabels();
        }
    }

    function toggleAutoRotate() {
        if (typeof NotesGraph3D !== 'undefined') {
            NotesGraph3D.toggleAutoRotate();
        }
    }

    function resetView() {
        if (typeof NotesGraph3D !== 'undefined') {
            NotesGraph3D.resetView();
        }
    }
