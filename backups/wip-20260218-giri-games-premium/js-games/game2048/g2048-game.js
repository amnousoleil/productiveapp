...
const G2048Game = (function() {
    ...
    function restart() {
        const board = container ? container.querySelector('.g2048-board') : null;
        if (board) { const ov = board.querySelector('div[style*="position:absolute"]'); if (ov) ov.remove(); }
    ...
    function render() {
        ...
        container.innerHTML = `<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:16px;">
            <div style="display:flex;gap:16px;align-items:center;flex-wrap:wrap;justify-content:center;">
                <div style="background:#1e293b;border:2px solid #334155;border-radius:10px;padding:10px 20px;text-align:center;min-width:80px;">
                    <div style="font-size:10px;color:#64748b;text-transform:uppercase;letter-spacing:.8px;font-weight:700">Score</div>
                    <div id="g2048-score" style="font-size:22px;font-weight:800;color:#f8fafc">0</div>
                </div>
                <div style="background:#1e293b;border:2px solid #334155;border-radius:10px;padding:10px 20px;text-align:center;min-width:80px;">
                    <div style="font-size:10px;color:#64748b;text-transform:uppercase;letter-spacing:.8px;font-weight:700">Meilleur</div>
                    <div id="g2048-best" style="font-size:22px;font-weight:800;color:#a78bfa">${bestScore.toLocaleString('fr-FR')}</div>
                </div>
                <div style="display:flex;gap:8px;">
                    <button onclick="G2048Game.restart()" class="games-btn">🔄 Nouveau</button>
                    <button onclick="G2048Game.undo()" class="games-btn">↩ Annuler</button>
                </div>
            </div>
            <div class="g2048-board" id="g2048-board"></div>
            <div style="font-size:13px;color:#64748b;text-align:center">⬆️⬇️⬅️➡️ Flèches · WASD · Swipe mobile</div>
        </div>`;
        renderBoard();
    }
