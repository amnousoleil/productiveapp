...
const GiriVisionView = {
  ...
  async saveRecording() {
    // ALWAYS download to Downloads folder automatically
    const blob = new Blob(this.recordedChunks, { type: 'video/webm' });
    const duration = Math.floor((Date.now() - (this.recordingStartTime || Date.now())) / 1000);
    const roomName = this.currentMeeting?.room_name || 'meeting';
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5);
    const filename = `giri-vision-${roomName}-${timestamp}.webm`;

    // Download file to Downloads folder (browser's default Downloads location)
    ...
    // Show success toast
    if (window.Toast) {
      Toast.success(`📥 Enregistrement téléchargé : ${filename}`);
    }

    // Try to also upload to backend (best effort, non-blocking)
    try {
      ...
      if (token && workspaceId) {
        ...
        if (response.ok) {
          console.log('[Giri Vision] Recording also saved to backend');
        }
      }
    } catch (err) {
      // Silent fail - download already succeeded
      console.warn('[Giri Vision] Backend upload failed (non-critical):', err);
    }
