...
export class AdminService {
  ...
  async getSystemHealth(): Promise<SystemHealth> {
    ...
    const status: 'healthy' | 'degraded' | 'down' =
      dbStatus === 'connected' && responseTime < 1000 ? 'healthy' :
  ...
  async getSystemStats(): Promise<SystemStats> {
    // Count total users
    const usersResult = await this.pool.query<{ count: string }>(
      'SELECT COUNT(*) as count FROM users'
    );
    const totalUsers = parseInt(usersResult.rows[0]?.count || '0', 10);

    // Count users active today (logged in today)
    const activeUsersResult = await this.pool.query<{ count: string }>(
    ...
    // Count workspaces
    const workspacesResult = await this.pool.query<{ count: string }>(
      'SELECT COUNT(*) as count FROM workspaces'
    );
    const totalWorkspaces = parseInt(workspacesResult.rows[0]?.count || '0', 10);

    // Count notes
    const notesResult = await this.pool.query<{ count: string }>(
      'SELECT COUNT(*) as count FROM notes'
    );
    const notesCount = parseInt(notesResult.rows[0]?.count || '0', 10);

    // Count tasks
    const tasksResult = await this.pool.query<{ count: string }>(
      'SELECT COUNT(*) as count FROM tasks'
    );
    const tasksCount = parseInt(tasksResult.rows[0]?.count || '0', 10);

    // Count projects
    const projectsResult = await this.pool.query<{ count: string }>(
    ...
    const loginsTodayResult = await this.pool.query<{ count: string }>(
      `SELECT COUNT(DISTINCT user_id) as count
       FROM user_gamification
       WHERE last_activity_at >= CURRENT_DATE`
    );
    ...
    const loginsWeekResult = await this.pool.query<{ count: string }>(
      `SELECT COUNT(DISTINCT user_id) as count
       FROM user_gamification
  ...
  async getMemberActivity(): Promise<MemberActivity[]> {
    const result = await this.pool.query<{
      user_id: string;
      name: string;
      last_activity_at: string | null;
      login_count_week: string;
      notes_count: string;
      tasks_count: string;
  ...
  async getRecentActivity(limit: number = 20): Promise<RecentActivity[]> {
    ...
    return result.rows.map((row) => ({
      type: row.type as 'login' | 'note' | 'task' | 'project',
