...
var AnimationControls = (function() {
    ...
    function buildPanelHTML() {
        ...
        return '<p class="anim-ctrl-panel-title">' + SVG_SPARKLE + ' Animations</p>' +
            ...
            '</div>' +
            '<div class="anim-ctrl-divider"></div>' +
            '<button id="anim-open-selector-btn" style="width:100%;padding:10px 14px;background:linear-gradient(135deg,rgba(124,58,237,0.25),rgba(99,102,241,0.25));border:1px solid rgba(138,99,246,0.4);border-radius:10px;color:#c4b5fd;cursor:pointer;font-size:13px;font-weight:600;display:flex;align-items:center;justify-content:center;gap:8px;">' +
    ...
    function bindPanelEvents() {
        ...
        // Sélecteur d'animations
        var selectorBtn = panelEl.querySelector('#anim-open-selector-btn');
        if (selectorBtn) {
            selectorBtn.addEventListener('click', function(e) {
                e.stopPropagation();
                if (window.AnimationSelector) window.AnimationSelector.open();
            });
        }
    }
