...
exports.NotesGraphService = void 0;
...
class NotesGraphService {
     ...
     */
    async buildGraph(workspaceId, includeManual = true, includeAuto = true) {
    ...
    `;
        ...
        // Manual links
        if (includeManual) {
            const manualLinks = await (0, database_js_1.sql) `
        SELECT source_note_id as source, target_note_id as target
        FROM note_links
        WHERE source_note_id IN ${(0, database_js_1.sql)(notes.map(n => n.id))}
      `;
        ...
        // Auto links
        if (includeAuto) {
            const autoLinks = await (0, database_js_1.sql) `
        SELECT
     ...
     */
    async autoLinkWorkspace(workspaceId, options = {}) {
        const startTime = Date.now();
        const { strategy: _strategy = 'keyword', // Reserved for embedding strategy
        minStrength = 0.3, maxLinksPerNote = 10 } = options;
    ...
    `;
        ...
        for (const pair of candidates) {
            ...
            const result = await this.aiService.analyzeRelationship(pair.noteA.id, pair.noteB.id);
            if (result && result.strength >= minStrength) {
                // Create link
                await (0, database_js_1.sql) `
          INSERT INTO note_auto_links ${(0, database_js_1.sql)({
     ...
     */
    findCandidatePairs(notes, minSimilarity = 0.2) {
        const pairs = [];
        for (let i = 0; i < notes.length; i++) {
            for (let j = i + 1; j < notes.length; j++) {
                ...
                const jaccard = this.aiService.calculateJaccard(noteA.keywords || [], noteB.keywords || []);
     ...
     */
    async clearAutoLinks(workspaceId) {
     ...
     */
    async classifyAllNotes(workspaceId, force = false) {
        // Get all notes
        const notes = await (0, database_js_1.sql) `
     ...
     */
    async generateKnowledgePath(workspaceId, createdBy, request) {
        // Generate path using AI
        const result = await this.aiService.generatePath(workspaceId, request.topic, request.maxNotes || 8);
        // Create knowledge path record
        const [path] = await (0, database_js_1.sql) `
      INSERT INTO knowledge_paths ${(0, database_js_1.sql)({
     ...
     */
    async getKnowledgePaths(workspaceId) {
        const paths = await (0, database_js_1.sql) `
     ...
     */
    async getCachedLayout(workspaceId) {
        const [layout] = await (0, database_js_1.sql) `
     ...
     */
    async saveLayout(workspaceId, layoutData, iterations = 100) {
        await (0, database_js_1.sql) `
      INSERT INTO note_graph_layout ${(0, database_js_1.sql)({
            workspace_id: workspaceId,
...
exports.NotesGraphService = NotesGraphService;
//# sourceMappingURL=notes-graph.service.js.map