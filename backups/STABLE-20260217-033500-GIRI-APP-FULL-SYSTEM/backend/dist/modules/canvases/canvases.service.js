...
exports.canvasesService = exports.CanvasesService = void 0;
...
class CanvasesService {
    async create(workspaceId, userId, input) {
        ...
        const now = new Date();
        const canvases = await (0, database_js_1.sql) `
    ...
    async getById(canvasId) {
        const canvases = await (0, database_js_1.sql) `SELECT * FROM canvases WHERE id = ${canvasId}`;
        if (canvases.length === 0) {
            throw helpers_js_1.AppError.notFound('Canvas');
        }
        return canvases[0];
    }
    async getByIdWithCollaborators(canvasId) {
        const canvases = await (0, database_js_1.sql) `
      SELECT c.*,
             json_build_object(
               'id', u.id,
    ...
    async list(workspaceId, userId, params) {
        ...
        let conditions = (0, database_js_1.sql) `c.workspace_id = ${workspaceId} AND (c.user_id = ${userId} OR c.is_public = true OR EXISTS (
      SELECT 1 FROM canvas_collaborators cc WHERE cc.canvas_id = c.id AND cc.user_id = ${userId}
    ))`;
        if (params.project_id !== undefined) {
            conditions = (0, database_js_1.sql) `${conditions} AND c.project_id IS NOT DISTINCT FROM ${params.project_id}`;
        }
        if (params.is_template !== undefined) {
            conditions = (0, database_js_1.sql) `${conditions} AND c.is_template = ${params.is_template}`;
        }
        const canvases = await (0, database_js_1.sql) `
    ...
    async update(canvasId, input) {
        ...
        if (input.elements !== undefined)
            updates.elements = JSON.stringify(input.elements);
        if (input.app_state !== undefined)
            updates.app_state = JSON.stringify(input.app_state);
    ...
    async delete(canvasId) {
        await (0, database_js_1.sql) `DELETE FROM canvases WHERE id = ${canvasId}`;
    }
    async addCollaborator(canvasId, input) {
    ...
    async updateLastAccessed(canvasId, userId) {
        await (0, database_js_1.sql) `
      UPDATE canvas_collaborators
    ...
    async canAccess(canvasId, userId) {
    ...
    async duplicate(canvasId, userId, workspaceId) {
...
exports.CanvasesService = CanvasesService;
exports.canvasesService = new CanvasesService();
//# sourceMappingURL=canvases.service.js.map