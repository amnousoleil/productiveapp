...
exports.SystemAlertService = void 0;
/**
...
class SystemAlertService {
    ...
    async createAlert(data) {
        try {
            const result = await this.pool.query(`INSERT INTO system_alerts (
    ...
    `;
      ...
      LIMIT $3 OFFSET $4
    `;
        const alertsResult = await this.pool.query(alertsQuery, [
            status || null,
            severity || null,
            limit,
            offset,
    ...
    async acknowledgeAlert(alertId) {
        try {
            const result = await this.pool.query(`UPDATE system_alerts
         SET status = 'acknowledged', acknowledged_at = NOW()
         WHERE id = $1 AND status = 'active'
         RETURNING id`, [alertId]);
    ...
    async resolveAlert(alertId, resolvedBy, resolutionNotes) {
        try {
         ...
         RETURNING id`, [alertId, resolvedBy, resolutionNotes || null]);
            return (result.rowCount || 0) > 0;
        }
        catch (error) {
     ...
     */
    async createAlertIfNotExists(alertType, data) {
        try {
            // Check si alerte active existe
            const existing = await this.pool.query(`SELECT id FROM system_alerts
    ...
    async cleanup() {
        try {
            const result = await this.pool.query(`DELETE FROM system_alerts
...
exports.SystemAlertService = SystemAlertService;
//# sourceMappingURL=system-alert.service.js.map