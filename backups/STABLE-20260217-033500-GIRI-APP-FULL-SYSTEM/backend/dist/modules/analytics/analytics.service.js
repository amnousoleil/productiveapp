...
class AnalyticsService {
    async logActivity(userId, workspaceId, input, ip, userAgent) {
        const id = (0, helpers_js_1.generateUUID)();
        const now = new Date();
        const activities = await (0, database_js_1.sql) `
    ...
    async getActivityLogs(workspaceId, params) {
        ...
        if (params.user_id) {
            conditions = (0, database_js_1.sql) `${conditions} AND user_id = ${params.user_id}`;
        }
        if (params.action) {
            conditions = (0, database_js_1.sql) `${conditions} AND action = ${params.action}`;
        }
        if (params.entity_type) {
            conditions = (0, database_js_1.sql) `${conditions} AND entity_type = ${params.entity_type}`;
        }
        const activities = await (0, database_js_1.sql) `
    ...
    async getActivitySummary(userId, workspaceId, dateRange) {
        const conditions = (0, database_js_1.sql) `user_id = ${userId} AND workspace_id = ${workspaceId} AND created_at >= ${dateRange.from} AND created_at <= ${dateRange.to}`;
        const total = await (0, database_js_1.sql) `
      SELECT COUNT(*)::int as count FROM activity_logs
      WHERE ${conditions}
    `;
        const byAction = await (0, database_js_1.sql) `
      SELECT action, COUNT(*)::int as count
      FROM activity_logs
      WHERE ${conditions}
      GROUP BY action
    `;
        const byEntity = await (0, database_js_1.sql) `
      SELECT entity_type, COUNT(*)::int as count
      FROM activity_logs
      WHERE ${conditions} AND entity_type IS NOT NULL
      GROUP BY entity_type
    `;
        const byHour = await (0, database_js_1.sql) `
    ...
    `;
        return {
            ...
            most_active_hours: byHour,
        };
    }
    async getDailyStats(userId, workspaceId, dateRange) {
    ...
    async updateDailyStats(userId, workspaceId) {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const tomorrow = new Date(today);
        tomorrow.setDate(tomorrow.getDate() + 1);
        // Calculate stats for today
        const notesCreated = await (0, database_js_1.sql) `
      SELECT COUNT(*)::int as count FROM notes
      WHERE user_id = ${userId} AND workspace_id = ${workspaceId}
        AND created_at >= ${today} AND created_at < ${tomorrow}
    `;
        const notesUpdated = await (0, database_js_1.sql) `
      SELECT COUNT(*)::int as count FROM notes
      WHERE user_id = ${userId} AND workspace_id = ${workspaceId}
    ...
    `;
        const messagesSent = await (0, database_js_1.sql) `
    ...
    `;
      ...
      WHERE user_id = ${userId} AND workspace_id = ${workspaceId}
        AND created_at >= ${today} AND created_at < ${tomorrow}
    `;
        const mostActiveHour = await (0, database_js_1.sql) `
    ...
    async getWorkspaceStats(workspaceId, dateRange) {
        const stats = await (0, database_js_1.sql) `
      SELECT *,
             TO_CHAR(date, 'YYYY-MM-DD') as date_formatted
    ...
    async updateWorkspaceStats(workspaceId) {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const totalUsers = await (0, database_js_1.sql) `
      SELECT COUNT(*)::int as count FROM workspace_members
      WHERE workspace_id = ${workspaceId}
    `;
        const activeUsers = await (0, database_js_1.sql) `
      SELECT COUNT(DISTINCT user_id)::int as count FROM activity_logs
      WHERE workspace_id = ${workspaceId}
        AND created_at >= ${new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000)}
    `;
        const notesCount = await (0, database_js_1.sql) `
      SELECT COUNT(*)::int as count FROM notes
      WHERE workspace_id = ${workspaceId} AND deleted_at IS NULL
    `;
        const tasksCount = await (0, database_js_1.sql) `
      SELECT COUNT(*)::int as count FROM tasks
      WHERE workspace_id = ${workspaceId}
    `;
        const messagesCount = await (0, database_js_1.sql) `
      SELECT COUNT(*)::int as count FROM messages m
      INNER JOIN conversations c ON m.conversation_id = c.id
    ...
    async getUserProductivityStats(userId, workspaceId) {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const tomorrow = new Date(today);
        tomorrow.setDate(tomorrow.getDate() + 1);
        ...
        // Today's stats
        const notesToday = await (0, database_js_1.sql) `
      SELECT COUNT(*)::int as count FROM notes
      WHERE user_id = ${userId} AND workspace_id = ${workspaceId}
        AND created_at >= ${today} AND created_at < ${tomorrow}
    `;
        const tasksToday = await (0, database_js_1.sql) `
      SELECT COUNT(*)::int as count FROM tasks
      WHERE user_id = ${userId} AND workspace_id = ${workspaceId}
        AND completed_at >= ${today} AND completed_at < ${tomorrow}
    `;
        const messagesToday = await (0, database_js_1.sql) `
      SELECT COUNT(*)::int as count FROM messages m
      INNER JOIN conversations c ON m.conversation_id = c.id
      WHERE m.sender_id = ${userId} AND c.workspace_id = ${workspaceId}
        AND m.created_at >= ${today} AND m.created_at < ${tomorrow}
    `;
        const xpToday = await (0, database_js_1.sql) `
      SELECT COALESCE(SUM(amount), 0)::int as total FROM xp_events
      WHERE user_id = ${userId} AND workspace_id = ${workspaceId}
        AND created_at >= ${today} AND created_at < ${tomorrow}
    `;
        const streak = await (0, database_js_1.sql) `
      SELECT current_streak FROM user_gamification
      WHERE user_id = ${userId} AND workspace_id = ${workspaceId}
    `;
        // Weekly comparison
        const notesThisWeek = await (0, database_js_1.sql) `
      SELECT COUNT(*)::int as count FROM notes
      WHERE user_id = ${userId} AND workspace_id = ${workspaceId}
        AND created_at >= ${weekAgo}
    `;
        const notesLastWeek = await (0, database_js_1.sql) `
      SELECT COUNT(*)::int as count FROM notes
      WHERE user_id = ${userId} AND workspace_id = ${workspaceId}
        AND created_at >= ${twoWeeksAgo} AND created_at < ${weekAgo}
    `;
        const tasksThisWeek = await (0, database_js_1.sql) `
      SELECT COUNT(*)::int as count FROM tasks
      WHERE user_id = ${userId} AND workspace_id = ${workspaceId}
        AND completed_at >= ${weekAgo}
    `;
        const tasksLastWeek = await (0, database_js_1.sql) `
      SELECT COUNT(*)::int as count FROM tasks
      WHERE user_id = ${userId} AND workspace_id = ${workspaceId}
        AND completed_at >= ${twoWeeksAgo} AND completed_at < ${weekAgo}
    `;
        const messagesThisWeek = await (0, database_js_1.sql) `
      SELECT COUNT(*)::int as count FROM messages m
      INNER JOIN conversations c ON m.conversation_id = c.id
      WHERE m.sender_id = ${userId} AND c.workspace_id = ${workspaceId}
        AND m.created_at >= ${weekAgo}
    `;
        const messagesLastWeek = await (0, database_js_1.sql) `
      SELECT COUNT(*)::int as count FROM messages m
      INNER JOIN conversations c ON m.conversation_id = c.id
      WHERE m.sender_id = ${userId} AND c.workspace_id = ${workspaceId}
        AND m.created_at >= ${twoWeeksAgo} AND m.created_at < ${weekAgo}
    `;
        const calcChange = (current, previous) => previous === 0 ? (current > 0 ? 100 : 0) : Math.round(((current - previous) / previous) * 100);
        return {
