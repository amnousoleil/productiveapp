...
exports.generateFEC = exports.initFECService = void 0;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
let pool;
const initFECService = (dbPool) => {
    pool = dbPool;
};
exports.initFECService = initFECService;
// ============================================
// COLONNES FEC
// ============================================
const FEC_HEADERS = [
 ...
 */
const getExpenseAccountCode = (categorySlug) => {
    const mapping = {
...
const getRevenueAccountCode = (categorySlug) => {
    const mapping = {
...
const getExpenseAccountLib = (categorySlug) => {
    const mapping = {
...
const getRevenueAccountLib = (categorySlug) => {
    const mapping = {
 ...
 */
const generateFEC = async (workspaceId, year) => {
    try {
        // Récupérer les paramètres de la société (SIRET)
        const settingsResult = await pool.query(`SELECT siret, company_name FROM company_settings
       WHERE workspace_id = $1`, [workspaceId]);
        ...
        const invoicesResult = await pool.query(`SELECT
       ...
       ORDER BY i.date_facture ASC, i.created_at ASC`, [workspaceId, year]);
        const entries = [];
        let ecritureNum = 1;
        ...
        for (const entry of entries) {
            const row = FEC_HEADERS.map(header => entry[header] || '');
            lines.push(row.join('\t'));
        }
       ...
       VALUES ($1, 'invoices', 'csv', $2, $3, 'completed')`, [workspaceId, filepath, JSON.stringify({ type: 'fec', year })]);
        return {
            filename,
            filepath,
...
exports.generateFEC = generateFEC;
// ============================================
 ...
 */
const getTVADeductibleAccount = (rate) => {
    if (rate === 20)
        return { code: '445661', lib: 'TVA déductible 20%' };
    if (rate === 10)
        return { code: '445662', lib: 'TVA déductible 10%' };
    if (rate === 5.5)
        return { code: '445663', lib: 'TVA déductible 5,5%' };
    if (rate === 2.1)
        return { code: '445664', lib: 'TVA déductible 2,1%' };
    return { code: '445660', lib: `TVA déductible ${rate}%` };
};
/**
 * Retourne le compte TVA collectée en fonction du taux
 */
const getTVACollectedAccount = (rate) => {
    if (rate === 20)
        return { code: '445711', lib: 'TVA collectée 20%' };
    if (rate === 10)
        return { code: '445712', lib: 'TVA collectée 10%' };
    if (rate === 5.5)
        return { code: '445713', lib: 'TVA collectée 5,5%' };
    if (rate === 2.1)
        return { code: '445714', lib: 'TVA collectée 2,1%' };
    return { code: '445710', lib: `TVA collectée ${rate}%` };
};
//# sourceMappingURL=accounting.fec.js.map