...
exports.getAlertCounts = exports.dismissAlert = exports.markAlertRead = exports.listAlerts = exports.generateAlerts = exports.initAlertsService = void 0;
let pool;
const initAlertsService = (dbPool) => {
    pool = dbPool;
};
exports.initAlertsService = initAlertsService;
// ============================================
 ...
 */
const generateAlerts = async (workspaceId) => {
    const generatedAlerts = [];
    try {
        // 1. Factures en retard (date_echeance dépassée, statut != paid)
        const overdueResult = await pool.query(`SELECT id, fournisseur, reference, montant_ttc, date_echeance,
       ...
       ORDER BY date_echeance ASC`, [workspaceId]);
        for (const inv of overdueResult.rows) {
            const severity = inv.days_overdue > 30 ? 'critical' : inv.days_overdue > 7 ? 'warning' : 'info';
        ...
        const upcomingResult = await pool.query(`SELECT id, fournisseur, reference, montant_ttc, date_echeance,
       ...
       ORDER BY date_echeance ASC`, [workspaceId]);
        for (const inv of upcomingResult.rows) {
            const severity = inv.days_until <= 1 ? 'warning' : 'info';
            const alert = await createAlertIfNotExists(workspaceId, {
       ...
       WHERE bl.workspace_id = $1 AND bl.year = $2 AND bl.month IS NULL`, [workspaceId, currentYear]);
 ...
 */
const listAlerts = async (workspaceId, unreadOnly) => {
    try {
        let whereClause = 'WHERE workspace_id = $1 AND is_dismissed = false';
        if (unreadOnly) {
            whereClause += ' AND is_read = false';
        }
        const result = await pool.query(`SELECT * FROM accounting_alerts
       ${whereClause}
       ORDER BY
         CASE severity WHEN 'critical' THEN 0 WHEN 'warning' THEN 1 ELSE 2 END,
         created_at DESC`, [workspaceId]);
        return result.rows;
    }
    catch (error) {
 ...
 */
const markAlertRead = async (workspaceId, alertId) => {
    try {
        const result = await pool.query(`UPDATE accounting_alerts
       SET is_read = true, updated_at = NOW()
       WHERE id = $1 AND workspace_id = $2`, [alertId, workspaceId]);
 ...
 */
const dismissAlert = async (workspaceId, alertId) => {
    try {
        const result = await pool.query(`UPDATE accounting_alerts
 ...
 */
const getAlertCounts = async (workspaceId) => {
    try {
...
exports.getAlertCounts = getAlertCounts;
/**
 ...
 */
const createAlertIfNotExists = async (workspaceId, data) => {
    // Vérifier si une alerte similaire existe déjà (non rejetée, < 24h)
    const existing = await pool.query(`SELECT id FROM accounting_alerts
     WHERE workspace_id = $1
       AND type = $2
       AND related_entity_id = $3
       AND is_dismissed = false
       AND created_at > NOW() - INTERVAL '24 hours'`, [workspaceId, data.type, data.related_entity_id]);
    if (existing.rows.length > 0) {
        return null; // Alerte déjà existante
    }
    const result = await pool.query(`INSERT INTO accounting_alerts (
...
//# sourceMappingURL=accounting.alerts.js.map