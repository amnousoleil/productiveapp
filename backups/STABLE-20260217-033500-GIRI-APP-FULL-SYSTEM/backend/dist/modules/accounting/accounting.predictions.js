...
exports.smartCategorize = exports.detectAnomalies = exports.predictCashFlow = exports.initPredictionsService = void 0;
const openai_1 = __importDefault(require("openai"));
let pool;
let openai = null;
const initPredictionsService = (dbPool, openaiApiKey) => {
    pool = dbPool;
    if (openaiApiKey) {
        openai = new openai_1.default({ apiKey: openaiApiKey });
    }
};
exports.initPredictionsService = initPredictionsService;
 ...
 */
const predictCashFlow = async (workspaceId, months = 6) => {
    try {
        // Récupérer les données historiques (12 derniers mois)
        const historicalResult = await pool.query(`SELECT
       ...
       WHERE workspace_id = $1`, [workspaceId]);
        ...
        const pendingResult = await pool.query(`SELECT
        type,
...
const detectAnomalies = async (workspaceId) => {
    try {
       ...
       GROUP BY type`, [workspaceId]);
        ...
        const outlierResult = await pool.query(`SELECT id, type, fournisseur, montant_ttc, date_facture, reference
       FROM invoices
       WHERE workspace_id = $1
       ...
       ORDER BY date_facture DESC`, [workspaceId]);
        ...
        const duplicateResult = await pool.query(`SELECT a.id as id_a, b.id as id_b,
       ...
       WHERE a.workspace_id = $1
         AND a.date_facture >= NOW() - INTERVAL '6 months'`, [workspaceId]);
 ...
 */
const smartCategorize = async (description, amount, categories) => {
    try {
        if (!openai) {
...
Réponds au format JSON:
{
  ...
  "alternatives": [{"category": "slug", "confidence": nombre}]
        ...
        const parsed = parseAIJsonResponse(content);
        return {
            suggested_category: parsed.suggested_category || categories[0]?.slug || 'autres-depenses',
            confidence: parsed.confidence || 50,
            reasoning: parsed.reasoning || 'Catégorisation par IA',
...
exports.smartCategorize = smartCategorize;
// ============================================
 ...
 */
const buildCashFlowPrompt = (historical, currentBalance, pending, months) => {
    const histStr = historical.map(h => `${h.month}/${h.year}: revenus=${h.income.toFixed(2)}, dépenses=${h.expense.toFixed(2)}, solde=${(h.income - h.expense).toFixed(2)}`).join('\n');
 ...
 */
const fallbackPrediction = (historical, currentBalance, months) => {
    const predictions = [];
    ...
    let riskLevel = 'low';
    if (lastPrediction) {
        if (lastPrediction.predicted_balance < 0)
            riskLevel = 'high';
        else if (lastPrediction.predicted_balance < currentBalance * 0.3)
            riskLevel = 'medium';
    }
 ...
 */
const keywordCategorize = (description, _amount, categories) => {
 ...
 */
const parseAIJsonResponse = (content) => {
    try {
...
//# sourceMappingURL=accounting.predictions.js.map