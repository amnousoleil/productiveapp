...
async function seed() {
  ...
  const userResult = await sql`SELECT id FROM users WHERE email = 'demo@productive.app'`;
  ...
  const workspaceResult = await sql`SELECT id FROM workspaces WHERE slug = 'demo-workspace'`;
  ...
  const projectResult = await sql`SELECT id FROM projects WHERE workspace_id = ${actualWorkspaceId} AND name = 'Getting Started'`;
