...
export const initBankService = (dbPool: Pool): void => {
  pool = dbPool;
};
...
): Promise<{ imported: number; batch_id: string }> => {
      ...
      const duplicateCheck = await client.query(
        `SELECT id FROM bank_transactions
         WHERE workspace_id = $1
           ...
           AND amount = $3
           AND description = $4
           AND COALESCE(bank_account_name, '') = COALESCE($5, '')`,
        [workspaceId, tx.transaction_date, tx.amount, tx.description, tx.bank_account_name || '']
      );
...
export const matchTransaction = async (
  ...
  try {
    ...
    const invoiceCheck = await pool.query(
      `SELECT id, montant_ttc FROM invoices WHERE id = $1 AND workspace_id = $2`,
    ...
    const result = await pool.query<BankTransaction>(
      `UPDATE bank_transactions
       ...
       RETURNING *`,
      [invoiceId, transactionId, workspaceId]
    );
...
export const autoMatchTransactions = async (
  ...
  try {
    ...
    const availableInvoices = await pool.query(
      `SELECT i.id, i.montant_ttc, i.date_facture, i.reference, i.fournisseur, i.type
       FROM invoices i
    ...
    for (const tx of unmatchedTx.rows) {
      let bestMatch: { invoiceId: string; confidence: number; reason: string } | null = null;

      for (const inv of invoices) {
        let confidence = 0;
        const reasons: string[] = [];
        const txAmount = Math.abs(parseFloat(tx.amount as unknown as string));
      ...
      if (bestMatch && bestMatch.confidence >= 60) {
        ...
        if (bestMatch.confidence >= 85) {
          await pool.query(
            ...
            [bestMatch.invoiceId, bestMatch.confidence, tx.id, workspaceId]
          );
