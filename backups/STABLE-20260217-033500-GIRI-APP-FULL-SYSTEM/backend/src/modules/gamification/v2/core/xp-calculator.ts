...
export type ActionType =
  ...
  | 'task_completed_easy' | 'task_completed_medium' | 'task_completed_hard' | 'task_completed_epic'
  | 'streak_bonus' | 'achievement' | 'daily_login' | 'daily_goal' | 'weekly_goal';
 ...
 */
export function calculateXP(options: CalculateOptions): XPCalculation {
  const { actionType, difficulty = 5, tags = [], currentStreak = 0, isFirstOfDay = false,
    hourOfDay = new Date().getHours(), comboCount = 0, prestigeLevel = 0, eventMultiplier = 1 } = options;
 ...
 */
export function estimateXP(actionType: ActionType, difficulty = 5): number {
  const baseXP = BASE_XP[actionType] ?? 10;
  const difficultyMult = 1 + (difficulty - 1) * 0.111;
