...
export class APIMetricsService {
   ...
   */
  getTopEndpoints(limit: number = 10): EndpointMetric[] {
  ...
  async saveSnapshot(): Promise<void> {
    try {
      ...
      await this.pool.query(
        `INSERT INTO health_checks (status, database_status, memory_used_mb, metadata)
         VALUES ($1, $2, $3, $4)`,
        ['healthy', 'ok', 0, JSON.stringify({ api_metrics: metrics })]
      );
    } catch (error) {
