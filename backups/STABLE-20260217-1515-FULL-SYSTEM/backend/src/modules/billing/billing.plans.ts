...
export interface PlanFeatures {
  // Notes
  maxNotes: number;          // -1 = illimité
  aiClustering: boolean;
  graph3D: boolean;
  // Prompt Forge
  promptsPerDay: number;     // -1 = illimité
  promptLibrary: boolean;
  // Général
  customDomains: number;     // -1 = illimité
  teamMembers: number;       // -1 = illimité
  storage: string;
 ...
 */
export function checkLimit(planId: PlanId, limitKey: keyof PlanFeatures, currentUsage: number) {
  const plan = PLANS[planId];
  if (!plan) return { allowed: false, remaining: 0, limit: 0 };
...
export function getPlanFromPriceId(priceId: string): PlanId {
  for (const [planId, plan] of Object.entries(PLANS) as [PlanId, Plan][]) {
...
export function isYearlyPrice(priceId: string): boolean {
  for (const plan of Object.values(PLANS)) {
    if (plan.stripePriceIdYearly === priceId) return true;
  }
  return false;
}
