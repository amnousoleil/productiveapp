...
import type { InboundEmail, ResendWebhookPayload, ResendEmailDetails, EmailFilters, EmailStats } from './mail-inbound.types';
...
export class MailInboundService {
   ...
   */
  static async processInboundEmail(webhookData: ResendWebhookPayload): Promise<void> {
   ...
   */
  private static async fetchEmailDetails(emailId: string): Promise<ResendEmailDetails | null> {
    try {
   ...
   */
  private static async findUserByEmailAddress(emailAddress: string): Promise<string | null> {
    const query = `
   ...
   */
  private static parseInboundEmail(details: ResendEmailDetails): InboundEmail {
   ...
   */
  private static generateThreadId(subject: string, headers?: Record<string, string>): string {
    ...
    return crypto.createHash('sha256').update(cleanSubject).digest('hex').substring(0, 16);
  }

  /**
   * Stocker l'email en DB
   */
  private static async storeInboundEmail(userId: string, email: InboundEmail): Promise<void> {
    const query = `
      INSERT INTO emails (
        user_id,
        resend_email_id,
   ...
   */
  static async getInbox(filters: EmailFilters): Promise<any[]> {
    ...
    const values: any[] = [user_id, folder];
    let paramIndex = 3;
   ...
   */
  static async markAsRead(emailId: string, userId: string, isRead: boolean): Promise<void> {
    const query = `
      UPDATE emails
    ...
    await pool.query(query, [isRead, emailId, userId]);
  }

  /**
   * Marquer un email comme favori
   */
  static async markAsStarred(emailId: string, userId: string, isStarred: boolean): Promise<void> {
    const query = `
      UPDATE emails
    ...
    await pool.query(query, [isStarred, emailId, userId]);
  }

  /**
   * Déplacer un email dans un dossier
   */
  static async moveToFolder(emailId: string, userId: string, folder: string): Promise<void> {
    const query = `
      UPDATE emails
    ...
    await pool.query(query, [folder, emailId, userId]);
  }
  ...
  static async deleteEmail(emailId: string, userId: string): Promise<void> {
    ...
    await pool.query(query, [emailId, userId]);
  }

  /**
   * Récupérer les stats d'un utilisateur
   */
  static async getUserStats(userId: string): Promise<EmailStats> {
    const query = `
      SELECT
