...
export async function computeProfile(userId: string, workspaceId: string): Promise<BehavioralProfile> {
  ...
  const hourCounts: { hour: number; count: number }[] = hourlyActivity.map(r => ({
  ...
  const projectEngagement: Record<string, number> = {};
  for (const row of projectActivity) {
    if (row.project_name) {
      projectEngagement[row.project_name as string] = Number(row.count);
    }
  }
...
export async function getStoredProfile(userId: string): Promise<BehavioralProfile | null> {
  ...
  return {
    ...
    peak_activity_hours: row.peak_activity_hours as number[],
    low_activity_hours: row.low_activity_hours as number[],
    ...
    project_engagement: row.project_engagement as Record<string, number>,
