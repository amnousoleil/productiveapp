...
exports.LifeInsightsService = void 0;
const pool_js_1 = require("./pool.js");
class LifeInsightsService {
    // ==================== Activity Logging ====================
    /**
     * Enregistre une nouvelle activité utilisateur
     */
    static async logActivity(dto) {
        const query = `
    ...
    `;
        ...
        const result = await pool_js_1.pool.query(query, values);
    ...
    `;
        ...
        const result = await pool_js_1.pool.query(sql, values);
        return result.rows;
    }
    // ==================== Statistics ====================
    /**
     ...
     */
    static async getActivityStats(query) {
        let dateFilter = '';
        const values = [query.user_id];
     ...
     */
    static async getHourlyDistribution(userId, days = 30) {
        const query = `
      SELECT
      ...
      ORDER BY hour
    `;
        const result = await pool_js_1.pool.query(query, [userId]);
        return result.rows.map((row) => ({
            hour: parseInt(row.hour),
            action_count: parseInt(row.action_count),
            dominant_action_type: row.dominant_action_type,
        }));
    }
    /**
     ...
     */
    static async getInsights(userId, memberId) {
        let query = `
    ...
    static async markInsightAsRead(insightId) {
        await pool_js_1.pool.query('UPDATE behavioral_insights SET is_read = true WHERE id = $1', [insightId]);
    }
    // ==================== Patterns Management ====================
    /**
     * Récupère les patterns actifs d'un utilisateur
     */
    static async getPatterns(userId, memberId) {
        let query = `
    ...
    `;
        const existing = await pool_js_1.pool.query(existingQuery, [pattern.user_id, pattern.pattern_type]);
        ...
        RETURNING *
      `;
            const result = await pool_js_1.pool.query(updateQuery, [
        ...
        RETURNING *
      `;
            const result = await pool_js_1.pool.query(insertQuery, [
                pattern.user_id,
     ...
     */
    static async getTodaySnapshot(userId, memberId) {
        const query = `
     ...
     */
    static async updateDailySnapshot(snapshot) {
        const query = `
     ...
     */
    static async getProfile(userId, memberId) {
    ...
    static async upsertProfile(profile) {
        ...
        RETURNING *
      `;
            const result = await pool_js_1.pool.query(query, [
     ...
     */
    static async exportUserData(userId, memberId) {
        const [activities, insights, patterns, snapshots, profile, stats] = await Promise.all([
            ...
            pool_js_1.pool.query('SELECT * FROM daily_snapshots WHERE user_id = $1 ORDER BY snapshot_date DESC LIMIT 90', [userId]),
...
exports.LifeInsightsService = LifeInsightsService;
//# sourceMappingURL=life-insights.service.js.map