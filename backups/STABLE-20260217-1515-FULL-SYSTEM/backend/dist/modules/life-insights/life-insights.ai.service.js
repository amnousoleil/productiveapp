...
class LifeInsightsAIService {
    ...
    static async analyzeUser(request) {
        ...
        const activities = await life_insights_service_js_1.LifeInsightsService.getActivities({
            user_id,
            member_id,
            start_date: new Date(Date.now() - days_to_analyze * 24 * 60 * 60 * 1000),
            limit: 5000,
        });
        const hourlyDist = await life_insights_service_js_1.LifeInsightsService.getHourlyDistribution(user_id, days_to_analyze);
        const dailyTrends = await life_insights_service_js_1.LifeInsightsService.getDailyTrends(user_id, days_to_analyze);
        const stats = await life_insights_service_js_1.LifeInsightsService.getActivityStats({ user_id, member_id, period: 'month' });
     ...
     */
    static async generatePsychologicalProfile(userId, memberId, activities, hourlyDist, dailyTrends, regenerate = false) {
        // Vérifier si existe déjà
        if (!regenerate) {
            const existing = await life_insights_service_js_1.LifeInsightsService.getProfile(userId, memberId);
...
Réponds UNIQUEMENT en JSON valide, format:
{
  ...
  "recommendations": ["<recommandations>"]
        ...
        return await life_insights_service_js_1.LifeInsightsService.upsertProfile(profile);
    }
    // ==================== Behavioral Insights ====================
    /**
     * Génère des insights comportementaux
     */
    static async generateBehavioralInsights(userId, memberId, activities, hourlyDist, dailyTrends) {
        const context = this.buildActivityContext(activities, hourlyDist, dailyTrends);
...
[{
  ...
  "evidence_count": <nombre>
        ...
        for (const data of insightsData) {
            const insight = await life_insights_service_js_1.LifeInsightsService.createInsight({
                user_id: userId,
                member_id: memberId || null,
     ...
     */
    static async detectPatterns(userId, memberId, activities, hourlyDist, dailyTrends) {
        const patterns = [];
        // 1. Peak hours pattern
        const peakHours = hourlyDist.sort((a, b) => b.action_count - a.action_count).slice(0, 3);
        if (peakHours.length > 0) {
            const pattern = await life_insights_service_js_1.LifeInsightsService.upsertPattern({
                user_id: userId,
                member_id: memberId || null,
        ...
        const completionRate = activities.filter((a) => a.action_type === 'task_completed').length /
        ...
        if (totalEnergy > 0) {
            ...
            if (morningEnergy / totalEnergy > 0.5)
                energyType = 'morning_person';
            else if (eveningEnergy / totalEnergy > 0.4)
     ...
     */
    static async generateDailySnapshot(userId, memberId, activities) {
...
Réponds en JSON:
{
  ...
  "productivity_score": <0-100>
        ...
        return await life_insights_service_js_1.LifeInsightsService.updateDailySnapshot(snapshot);
    }
    // ==================== Recommendations ====================
    /**
     * Génère des recommandations personnalisées
     */
    static async generateRecommendations(profile, insights, patterns) {
     ...
     */
    static buildActivityContext(activities, hourlyDist, dailyTrends) {
        const actionTypeCounts = {};
...
`.trim();
     ...
     */
    static calculateConfidenceScore(dataPoints, daysAnalyzed) {
    ...
    static async callOpenAI(prompt) {
        ...
        const messages = [
            {
                role: 'system',
                content: 'Tu es un psychologue expert en analyse comportementale. Tu réponds toujours en JSON valide et structuré. Tes analyses sont précises, empathiques et actionnables.',
            },
            {
                role: 'user',
                content: prompt,
...
exports.LifeInsightsAIService = LifeInsightsAIService;
//# sourceMappingURL=life-insights.ai.service.js.map