...
class JournalService {
     ...
     */
    async getEntries(userId, workspaceId, filters = {}) {
        let query = `
     ...
     */
    async getEntryById(entryId, userId, workspaceId) {
        const result = await this.pool.query(`SELECT * FROM journal_entries
       WHERE id = $1 AND user_id = $2 AND workspace_id = $3`, [entryId, userId, workspaceId]);
        return result.rows[0] || null;
    }
    /**
     * Get journal entry for a specific date (unique per user/workspace/date)
     */
    async getEntryByDate(userId, workspaceId, date) {
        const result = await this.pool.query(`SELECT * FROM journal_entries
       WHERE user_id = $1 AND workspace_id = $2 AND date = $3`, [userId, workspaceId, date]);
        return result.rows[0] || null;
    }
    /**
     * Create or update journal entry for a specific date
     */
    async upsertEntry(userId, workspaceId, data) {
        ...
        const result = await this.pool.query(`INSERT INTO journal_entries (
     ...
     */
    async updateEntry(entryId, userId, workspaceId, data) {
        const updates = [];
        const params = [];
        let paramIndex = 1;
        ...
        const result = await this.pool.query(`UPDATE journal_entries
     ...
     */
    async deleteEntry(entryId, userId, workspaceId) {
        const result = await this.pool.query(`DELETE FROM journal_entries
       WHERE id = $1 AND user_id = $2 AND workspace_id = $3`, [entryId, userId, workspaceId]);
     ...
     */
    async getStatistics(userId, workspaceId, startDate, endDate) {
        let query = `
      SELECT
