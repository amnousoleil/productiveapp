...
exports.getTVASummary = exports.getMonthlyAnalytics = exports.getDashboardStats = exports.initAnalyticsService = void 0;
let pool;
const initAnalyticsService = (dbPool) => {
    pool = dbPool;
};
...
// DASHBOARD STATS
// ============================================
const getDashboardStats = async (workspaceId, year) => {
    ...
    const totalsResult = await pool.query(`SELECT
     ...
     FROM invoices
     WHERE workspace_id = $1 AND EXTRACT(YEAR FROM date_facture) = $2`, [workspaceId, targetYear]);
    const totals = totalsResult.rows[0];
    const totalIncome = parseFloat(totals.total_income);
    const totalExpense = parseFloat(totals.total_expense);
    // Top catégories
    const topCategoriesResult = await pool.query(`SELECT
      c.id as category_id,
     ...
     HAVING COUNT(i.id) > 0
     ORDER BY total DESC
     LIMIT 10`, [workspaceId, targetYear]);
    // Tendance mensuelle
    const trendResult = await pool.query(`SELECT
     ...
     ORDER BY year, month`, [workspaceId, targetYear]);
    return {
...
// ANALYTICS MENSUEL
// ============================================
const getMonthlyAnalytics = async (workspaceId, month, year) => {
    // Revenus par catégorie
    const incomeResult = await pool.query(`SELECT
     ...
     GROUP BY c.id, c.name, c.type`, [workspaceId, month, year]);
    ...
    const expenseResult = await pool.query(`SELECT
     ...
     GROUP BY c.id, c.name, c.type`, [workspaceId, month, year]);
    // TVA par taux
    const tvaResult = await pool.query(`SELECT
      tva_rate as taux,
     ...
     GROUP BY tva_rate`, [workspaceId, month, year]);
    // Mois précédent pour comparaison
    const prevMonth = month === 1 ? 12 : month - 1;
    const prevYear = month === 1 ? year - 1 : year;
    const prevResult = await pool.query(`SELECT
      COALESCE(SUM(CASE WHEN type = 'income' THEN montant_ttc ELSE 0 END), 0) as income,
      COALESCE(SUM(CASE WHEN type = 'expense' THEN montant_ttc ELSE 0 END), 0) as expense
     FROM invoices
     WHERE workspace_id = $1
       AND EXTRACT(MONTH FROM date_facture) = $2
       AND EXTRACT(YEAR FROM date_facture) = $3`, [workspaceId, prevMonth, prevYear]);
    const totalIncome = incomeResult.rows.reduce((sum, r) => sum + parseFloat(r.total), 0);
    const totalExpense = expenseResult.rows.reduce((sum, r) => sum + parseFloat(r.total), 0);
    ...
    return {
        ...
        tva: tvaResult.rows.map(r => ({
            ...
            solde: parseFloat(r.montant_collecte) -
                parseFloat(r.montant_deductible)
        })),
        comparison: {
...
// RÉSUMÉ TVA
// ============================================
const getTVASummary = async (workspaceId, year, quarter) => {
...
//# sourceMappingURL=accounting.analytics.js.map