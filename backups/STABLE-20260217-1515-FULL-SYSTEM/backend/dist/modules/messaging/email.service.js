...
exports.emailConversationSummary = emailConversationSummary;
exports.emailFullConversation = emailFullConversation;
 ...
 */
async function emailConversationSummary(conversationId, recipientEmail, exportedBy) {
    // Get conversation details
    const convResult = await database_1.default.query(`SELECT name, description, type
        FROM conversations
    ...
    const messagesResult = await database_1.default.query(`SELECT
            m.id,
            m.content,
            m.created_at,
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    `;
    // Send email
    await (0, mail_service_1.sendEmail)({
        to: recipientEmail,
        subject: `TeamTalk - Résumé: ${conversation.name || 'Conversation'}`,
        html,
    });
    // Log export
    await database_1.default.query(`INSERT INTO conversation_email_exports
        (conversation_id, exported_by, recipient_email, message_count, export_type)
        VALUES ($1, $2, $3, $4, 'summary')`, [conversationId, exportedBy, recipientEmail, messages.length]);
}
/**
 * Email full conversation (all messages)
 */
async function emailFullConversation(conversationId, recipientEmail, exportedBy) {
    // Get conversation details
    const convResult = await database_1.default.query(`SELECT name, description, type, created_at
        FROM conversations
        WHERE id = $1`, [conversationId]);
    if (convResult.rows.length === 0) {
        throw new Error('Conversation not found');
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 800px; margin: 0 auto; padding: 20px;">
    ...
    await database_1.default.query(`INSERT INTO conversation_email_exports
        (conversation_id, exported_by, recipient_email, message_count, export_type)
        VALUES ($1, $2, $3, $4, 'full')`, [conversationId, exportedBy, recipientEmail, messages.length]);
}
/**
 * Get export history for conversation
 */
async function getExportHistory(conversationId) {
