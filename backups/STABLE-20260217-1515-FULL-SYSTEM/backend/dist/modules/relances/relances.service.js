...
exports.getOverdueReport = exports.cancelReminder = exports.listReminders = exports.processReminders = exports.scheduleReminders = exports.updateSettings = exports.getSettings = exports.initRelancesService = void 0;
let pool;
const initRelancesService = (p) => { pool = p; };
exports.initRelancesService = initRelancesService;
...
const getSettings = async (wid) => {
    const r = await pool.query('SELECT * FROM reminder_settings WHERE workspace_id = $1', [wid]);
    if (r.rows[0])
        return r.rows[0];
    return (await pool.query(`INSERT INTO reminder_settings (workspace_id, enabled, level1_days, level1_subject, level1_template, level2_days, level2_subject, level2_template, level3_days, level3_subject, level3_template, sender_name, sender_email, auto_schedule)
     VALUES ($1, true, 7, $2, $3, 15, $4, $5, 30, $6, $7, 'Service Comptabilite', '', true) ON CONFLICT (workspace_id) DO UPDATE SET updated_at=NOW() RETURNING *`, [wid, DEFAULT_SUBJECTS[0], DEFAULT_BODIES[0], DEFAULT_SUBJECTS[1], DEFAULT_BODIES[1], DEFAULT_SUBJECTS[2], DEFAULT_BODIES[2]])).rows[0];
};
exports.getSettings = getSettings;
const updateSettings = async (wid, data) => {
    ...
    if (!fields.length)
        return (0, exports.getSettings)(wid);
    vals.push(wid);
    return (await pool.query(`UPDATE reminder_settings SET ${fields.join(', ')}, updated_at=NOW() WHERE workspace_id=$${pi} RETURNING *`, vals)).rows[0] || (await (0, exports.getSettings)(wid));
};
exports.updateSettings = updateSettings;
const scheduleReminders = async (wid) => {
    const settings = await (0, exports.getSettings)(wid);
    if (!settings.enabled)
        return { scheduled: 0, skipped: 0 };
    const overdue = (await pool.query(`SELECT i.id, i.reference, i.montant_ttc, i.date_echeance, i.client_email, c.email AS contact_email,
     ...
     WHERE i.workspace_id=$1 AND i.type='income' AND i.status NOT IN ('paid','cancelled','draft') AND i.date_echeance < NOW()`, [wid])).rows;
    ...
    for (const inv of overdue) {
        ...
        for (const { level, days } of [{ level: 1, days: settings.level1_days }, { level: 2, days: settings.level2_days }, { level: 3, days: settings.level3_days }]) {
            if (level <= maxLvl || inv.days_overdue < days)
                continue;
            ...
            const sa = new Date(inv.date_echeance);
...
const processReminders = async () => {
    const pending = (await pool.query(`SELECT ir.*, i.reference, i.montant_ttc, i.date_echeance, i.client_name, i.client_email, c.name AS contact_name,
     ...
     WHERE ir.status='pending' AND ir.scheduled_at <= NOW() LIMIT 50`)).rows;
    let sent = 0, failed = 0;
    for (const r of pending) {
        const email = r.email_to || r.client_email;
        if (!email) {
            await pool.query(`UPDATE invoice_reminders SET status='failed', error_message='Pas d''email', updated_at=NOW() WHERE id=$1`, [r.id]);
            failed++;
            continue;
        }
        const lvl = r.level - 1;
        ...
        const vars = { '{{reference}}': r.reference || 'N/A', '{{amount}}': parseFloat(r.montant_ttc || 0).toFixed(2), '{{due_date}}': new Date(r.date_echeance).toLocaleDateString('fr-FR'), '{{client_name}}': r.client_name || r.contact_name || 'Client', '{{company_name}}': r.sender_name || 'Notre societe', '{{days_overdue}}': String(Math.floor((Date.now() - new Date(r.date_echeance).getTime()) / 86400000)) };
...
const listReminders = async (wid, filters) => {
    const page = filters.page || 1, limit = Math.min(filters.limit || 20, 100), offset = (page - 1) * limit;
    let wc = 'WHERE ir.workspace_id = $1';
    const p = [wid];
    let pi = 2;
    if (filters.invoiceId) {
        wc += ` AND ir.invoice_id=$${pi++}`;
        p.push(filters.invoiceId);
    }
    if (filters.status) {
        wc += ` AND ir.status=$${pi++}`;
...
const cancelReminder = async (wid, id) => {
...
const getOverdueReport = async (wid) => {
    const invs = (await pool.query(`SELECT i.id, i.reference, i.fournisseur, i.client_name, i.montant_ttc, i.date_echeance,
     EXTRACT(DAY FROM NOW()-i.date_echeance)::int AS days_overdue,
     (SELECT COUNT(*) FROM invoice_reminders ir WHERE ir.invoice_id=i.id AND ir.status='sent')::int AS reminder_count
     FROM invoices i WHERE i.workspace_id=$1 AND i.type='income' AND i.status NOT IN ('paid','cancelled','draft') AND i.date_echeance < NOW() ORDER BY i.date_echeance`, [wid])).rows;
    const total = invs.reduce((s, i) => s + parseFloat(i.montant_ttc), 0);
    return { total_overdue: invs.length, total_amount: total, invoices: invs };
};
exports.getOverdueReport = getOverdueReport;
//# sourceMappingURL=relances.service.js.map