...
class AdminDashboardService {
    ...
    healthCheckService;
    systemAlertService;
    constructor(pool, apiMetricsService, errorLogService, healthCheckService, systemAlertService) {
        this.pool = pool;
    ...
    async getDatabaseMetrics() {
        // 1. Taille BDD
        const sizeResult = await this.pool.query(`SELECT ROUND(pg_database_size(current_database()) / 1024.0 / 1024.0, 2) as size_mb`);
        const sizeMb = parseFloat(sizeResult.rows[0]?.size_mb || '0');
        // 2. Nombre de tables
        const tableCountResult = await this.pool.query(`SELECT COUNT(*) as count FROM information_schema.tables WHERE table_schema = 'public'`);
        const tableCount = parseInt(tableCountResult.rows[0]?.count || '0', 10);
        // 3. Requêtes lentes (nécessite pg_stat_statements extension)
        let slowQueries = [];
        try {
            const slowQueriesResult = await this.pool.query(`SELECT
        ...
        const missingIndexesResult = await this.pool.query(`SELECT
    ...
    async getUserAnalytics(startDate, endDate) {
      ...
      ORDER BY ug.last_activity_at DESC NULLS LAST`, [start, end]);
        return result.rows.map((row) => ({
            ...
            totalActions: parseInt(row.notes_count, 10) +
    ...
    async parseChangelog() {
        ...
        const entries = [];
        try {
            const content = fs_1.default.readFileSync(memoryPath, 'utf-8');
