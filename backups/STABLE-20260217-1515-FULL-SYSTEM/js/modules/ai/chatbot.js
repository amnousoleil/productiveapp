...
const Chatbot = {
    ...
    buildContext() {
        ...
        if (document.body.classList.contains('galaxy-active')) {
            ctx += `📍 VUE ACTIVE: Galaxy View (canvas infini de mind mapping)\n`;
        }
        ctx += '\n';
...
`;
    ...
    async send() {
        ...
        // Commandes locales d'abord (stats, etc.)
        const localHandled = await this.handleLocalCommands(message);
        if (localHandled) return;

        const loadingDiv = this.addMessage('Traitement...', 'assistant loading');

        try {
            // NOUVEAU : Utiliser Mahayawen Agent v2.0 pour tout gérer
            if (typeof MahayawenAgent !== 'undefined') {
                console.log('🤖 Using Mahayawen Agent v2.0');

                const result = await MahayawenAgent.execute(message);
                ...
                if (result.success) {
                    ...
                    this.addMessage(result.message, 'assistant error');

                    // Afficher les suggestions si disponibles
                    if (result.suggestions && result.suggestions.length > 0) {
                        const suggestionsMsg = `\n\n💡 Suggestions:\n${result.suggestions.map(s => `- ${s}`).join('\n')}`;
                        this.addMessage(suggestionsMsg, 'assistant');
                    }
                }
            ...
            // FALLBACK : Ancien système si Mahayawen Agent n'est pas disponible
            let aiResponse;
            let hadActions = false;

            // Utiliser le nouveau backend AI (OpenAI direct avec smart routing)
            if (typeof ApiAi !== 'undefined' && ApiAi.isAvailable()) {
                ...
                // Fallback to N8N (legacy)
                console.log('⚠️ ApiAi not available, falling back to N8N');
    ...
    /**
     * Execute an action from the backend AI
     */
    ...
    /**
     * Injecte les éléments UI Mahayawen Voice dans le DOM
     */
    injectMahayawenUI() {
        // Vérifier si déjà injecté
        if (document.getElementById('mahayawen-voice-btn')) {
            return;
        }

        // Bouton vocal flottant
        const voiceBtn = document.createElement('button');
        voiceBtn.id = 'mahayawen-voice-btn';
        voiceBtn.className = 'voice-idle';
        voiceBtn.innerHTML = '🎤';
        voiceBtn.title = 'Commandes vocales (Ctrl+Shift+V)';
        voiceBtn.setAttribute('aria-label', 'Activer les commandes vocales');
        document.body.appendChild(voiceBtn);

        // Zone de transcript en temps réel
        const transcript = document.createElement('div');
        transcript.id = 'mahayawen-transcript';
        transcript.setAttribute('aria-live', 'polite');
        document.body.appendChild(transcript);

        console.log('✅ Mahayawen UI injected');
    },

    /**
     * Initialise le bouton vocal et ses événements
     */
    initVoiceButton() {
        const voiceBtn = document.getElementById('mahayawen-voice-btn');
        if (!voiceBtn || typeof MahayawenVoice === 'undefined') {
            return;
        }

        // Click pour toggle voice
        voiceBtn.addEventListener('click', () => {
            MahayawenVoice.toggle();
        });

        // Long press pour mode conduite
        let longPressTimer;
        voiceBtn.addEventListener('mousedown', () => {
            longPressTimer = setTimeout(() => {
                if (confirm('Activer le mode conduite (écoute continue) ?')) {
                    MahayawenVoice.enableDrivingMode();
                }
            }, 2000);
        });
    ...
    initEvents() {
        ...
        // Injecter les éléments UI Mahayawen Voice
        this.injectMahayawenUI();

        // Initialiser Mahayawen Agent v2.0
        if (typeof MahayawenAgent !== 'undefined') {
            setTimeout(() => {
                MahayawenAgent.init();
                console.log('🚀 Mahayawen Agent v2.0 ready!');
                console.log('📊 Capabilities:', MahayawenAgent.getCapabilitiesReport());
            }, 1000);
        }

        // Initialiser Voice Interface
        if (typeof MahayawenVoice !== 'undefined') {
            setTimeout(() => {
                const voiceEnabled = MahayawenVoice.init();
                if (voiceEnabled) {
                    console.log('🎤 Voice commands ready!');
                    console.log('💡 Say "Hey Mahayawen" followed by your command');
                    console.log('💡 Or use Ctrl+Shift+V to toggle voice mode');
                    MahayawenVoice.initKeyboardShortcuts();

                    // Connecter le bouton vocal
