...
const AccInvoices = (function() {
    ...
    function _renderTable(inv, tw) {
        if (!inv.length) {
            tw.innerHTML = '<div style="text-align:center;padding:60px 20px">' +
                ...
                '<h3 style="color:var(--text-primary);margin:0 0 8px">Aucune facture trouvee</h3>' +
                '<p style="color:var(--text-secondary);margin:0 0 20px">Creez votre premiere facture pour commencer a suivre vos finances.</p>' +
                '<button id="acc-inv-en" style="padding:8px 18px;border:none;border-radius:8px;background:var(--accent-primary);color:#fff;cursor:pointer;font-weight:600">+ Nouvelle facture</button></div>';
        ...
        var h = '<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:.9rem">' +
            ...
            '<th style="'+THS+'">Date</th><th style="'+THS+'">N&deg;</th>' +
            '<th style="'+THS+'">Fournisseur / Client</th><th style="'+THS+'">Categorie</th>' +
            '<th style="'+THS+'">Type</th><th style="'+THS+'text-align:right">Montant TTC</th>' +
        ...
        inv.forEach(function(i) {
            ...
            var sk = st.strike ? 'text-decoration:line-through;opacity:.6;' : '';

            h += '<tr data-id="'+i.id+'" class="acc-ir" style="border-bottom:1px solid var(--border-color);cursor:pointer;'+sk+'">' +
                '<td style="'+TDS+'">'+dt+'</td>' +
                '<td style="'+TDS+'font-family:monospace">'+_e(i.reference||'-')+'</td>' +
                '<td style="'+TDS+'">'+_e(i.contact_name||i.client_name||'-')+'</td>' +
                '<td style="'+TDS+'">'+_e(cn)+'</td>' +
                '<td style="'+TDS+'"><span style="display:inline-block;padding:2px 10px;border-radius:12px;font-size:.8rem;font-weight:600;color:'+tp.color+';background:'+tp.bg+'">'+tp.label+'</span></td>' +
                '<td style="'+TDS+'text-align:right;font-family:monospace;font-weight:600">'+EUR.format(i.total_ttc||0)+'</td>' +
    ...
    function _rowActs(i) {
        var s = i.status, b = '';
        b += '<button data-act="view" style="'+_abtn('var(--accent-primary)')+'">Voir</button>';
