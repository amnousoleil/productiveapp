...
function animate() {
    ...
    // Pour hacker et matrix: effet de traînée (fade progressif)
    // Pour les autres: clear complet (pas de traces)
    if (theme === 'hacker') {
        ...
        // Clear complet pour tous les autres thèmes
        matrixCtx.clearRect(0, 0, matrixCanvas.width, matrixCanvas.height);
    }
...
// === HACKER THEME - Rayons et glitch ===
function animateHacker() {
    matrixCtx.font = '14px monospace';
    matrixCtx.fillStyle = '#ffd700';
    
    // Caractères qui tombent (style matrix mais doré)
    const chars = '01アイウエオカキクケコサシスセソ';
    const columns = Math.floor(matrixCanvas.width / 20);
    
    for (let i = 0; i < columns; i++) {
        if (matrixDrops[i] === undefined) matrixDrops[i] = Math.random() * -100;
    ...
    // Rayons horizontaux aléatoires (glitch effect)
    if (Math.random() > 0.97) {
        const y = Math.random() * matrixCanvas.height;
        const gradient = matrixCtx.createLinearGradient(0, y, matrixCanvas.width, y);
...
// === MATRIX THEME - Code vert classique ===
...
function animateOcean() {
    ...
    particles.forEach(p => {
        ...
        matrixCtx.strokeStyle = `rgba(0, 180, 216, ${0.3 + p.size / 20})`;
        ...
        matrixCtx.fillStyle = 'rgba(255, 255, 255, 0.3)';
        matrixCtx.fill();
    });
}

// === FANTASY THEME - Étoiles et magie ===
function animateFantasy() {
    ...
    particles.forEach(p => {
        ...
        matrixCtx.fillStyle = p.color + Math.floor(twinkle * p.life * 255).toString(16).padStart(2, '0');
        
        // Dessiner une étoile
        matrixCtx.beginPath();
        for (let i = 0; i < 5; i++) {
            const angle = (i * 4 * Math.PI / 5) - Math.PI / 2;
            const x = p.x + Math.cos(angle) * p.size * twinkle;
            const y = p.y + Math.sin(angle) * p.size * twinkle;
...
// === SUNSET THEME - Particules chaudes ===
function animateSunset() {
    if (particles.length < 40 && Math.random() > 0.93) {
        particles.push({
            x: Math.random() * matrixCanvas.width,
            y: matrixCanvas.height + 10,
            ...
            life: 1,
            color: ['#f97316', '#fbbf24', '#dc2626'][Math.floor(Math.random() * 3)]
        });
    }
    ...
    particles.forEach(p => {
        ...
        matrixCtx.beginPath();
        matrixCtx.fillStyle = p.color + Math.floor(p.life * 150).toString(16).padStart(2, '0');
        matrixCtx.shadowBlur = 10;
        matrixCtx.shadowColor = p.color;
...
// === DEFAULT (Desert et autres) - Particules douces ===
function animateDefault() {
    ...
    const accentColor = getComputedStyle(document.documentElement).getPropertyValue('--accent').trim() || '#e07840';
    ...
    particles.forEach(p => {
        ...
        matrixCtx.beginPath();
        matrixCtx.fillStyle = accentColor + Math.floor(p.life * 80).toString(16).padStart(2, '0');
        matrixCtx.arc(p.x, p.y, p.size * p.life, 0, Math.PI * 2);
        matrixCtx.fill();
    });
}
...
function animate() {
    ...
    // Effet de fade au lieu de clear complet
    matrixCtx.fillStyle = theme === 'hacker' ? 'rgba(0, 0, 0, 0.05)' : 
                          theme === 'matrix' ? 'rgba(10, 15, 10, 0.05)' : 
                          'rgba(26, 20, 16, 0.1)';
    matrixCtx.fillRect(0, 0, matrixCanvas.width, matrixCanvas.height);
